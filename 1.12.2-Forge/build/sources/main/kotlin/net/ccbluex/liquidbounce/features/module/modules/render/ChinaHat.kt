...
@ModuleInfo(name = "ChinaHat", description = "China!",category = ModuleCategory.RENDER)
...
private val rainbowStartValue = FloatValue("RainbowStart", 0.41f, 0f, 1f)
private val rainbowStopValue = FloatValue("RainbowStop", 0.58f, 0f, 1f)

@EventTarget
    fun onRender3d(event: Render3DEvent) {

            if(drawThePlayerValue.get() && !onlyThirdPersonValue.get() ) {
            drawChinaHatFor(mc.thePlayer!!)
            }
            if(drawTargetsValue.get()) {
            mc.theWorld!!.loadedEntityList.forEach {
            if(EntityUtils.isSelected(it, true)) {
            drawChinaHatFor(it as IEntityPlayerSP)
            }
       }
    }
}

private fun drawChinaHatFor(entity: IEntityPlayerSP) {
        GL11.glPushMatrix()
        ...
        if(colorRainbowValue.get()) {
                RenderUtils.glColor(Color.getHSBColor(if (i <180) {rainbowStartValue.get() + (rainbowStopValue.get() - rainbowStartValue.get()) * (i / 180f) } else { rainbowStartValue.get() + (rainbowStopValue.get() - rainbowStartValue.get()) * (-(i-360) / 180f) }, 0.7f, 1.0f), colorAlphaValue.get() / 255f)
        }
