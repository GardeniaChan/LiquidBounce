...
class BlockFly : Module() {
    ...
    private val rotationModeValue = ListValue("RotationMode", arrayOf("Vanilla","Test"),"Test")
    ...
    private fun setRotation(rotation: Rotation) {
        ...
        if (silentRotationValue.get()) {
            if (rotationModeValue.equals("Vanilla")) {
                RotationUtils.setTargetRotation(rotation, keepLengthValue.get())
            } else {
                val rotationT = Rotation(400.toFloat(),80.toFloat())
                RotationUtils.setTargetRotation(rotationT,0)
            }
        } else {
